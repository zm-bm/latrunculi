...
5. Recheck applicable objective, legal, and protocol guards; run the benchmark
   ...
   paired variants in their predeclared literal serial order. A missing,
...
1. For explicitly authorized paired strength testing, use OpenBench when
   ...
   runner/book hashes, identical paired settings and adjudication, an enforced
   game cap, raw log, PGN, pentanomial counts, and the SPRT result.
2. Predeclare the hypotheses and the guide's OpenBench-enforced game budget.
   Local matches need the same explicit hypothesis and enforced cap. Less than
   the default is allowed; more needs explicit user authorization. A missing or
   unenforced cap blocks testing. Use SPRT for candidate strength by default;
   use fixed games only when a fixed sample is itself the objective.
3. Run one match at a time. Confirm its identity, revisions or binary hashes,
   settings, mode, positive limit, bounds or fixed-game count, and running state
   once; record them and return control without polling. Inspect status again
   only in a later user request or goal continuation. Interpret upper bound as
   acceptance, lower bound as rejection, and a cap reached inside both bounds
   as inconclusive; further testing needs separate justification and
   authorization.
