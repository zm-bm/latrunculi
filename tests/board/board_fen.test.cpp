...
constexpr std::array<std::string_view, 6> round_trip_fens = {
...
TEST(BoardFenTest, RoundTripsRepresentativePositions) {
    for (const std::string_view fen : round_trip_fens) {
        SCOPED_TRACE(fen);
        Board board(fen);
        EXPECT_EQ(board.to_fen(), fen);

        Board reloaded(board.to_fen());
...
TEST(BoardFenTest, FourFieldFenNormalizesClocks) {
    EXPECT_EQ(Board("4k3/8/8/8/8/8/8/4K3 w - -").to_fen(), board_test::fen::kings_only);
    EXPECT_EQ(Board("4k3/8/8/8/8/8/8/4K3 b - -").to_fen(), "4k3/8/8/8/8/8/8/4K3 b - - 0 1");
}
...
TEST(BoardFenTest, LoadingFenAfterLongHistoryStartsAFreshUsableRoot) {
    Board board;
