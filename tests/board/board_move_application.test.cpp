...
TEST(BoardMoveApplicationTest, DisablesAllCastlingRights) {
    board_test::Harness board(board_test::fen::start);
    board.disable_castle(WHITE);
    EXPECT_FALSE(board.can_castle(WHITE));
    board.disable_castle(BLACK);
    EXPECT_FALSE(board.can_castle(BLACK));
}

TEST(BoardMoveApplicationTest, DisablesKingsideCastlingRights) {
    board_test::Harness board(board_test::fen::start);
    board.disable_castle(WHITE, H1);
    EXPECT_TRUE(board.can_castle(WHITE));
    EXPECT_FALSE(board.can_castle_kingside(WHITE));
    EXPECT_TRUE(board.can_castle_queenside(WHITE));
    board.disable_castle(BLACK, H8);
    EXPECT_TRUE(board.can_castle(BLACK));
    EXPECT_FALSE(board.can_castle_kingside(BLACK));
    EXPECT_TRUE(board.can_castle_queenside(BLACK));
}

TEST(BoardMoveApplicationTest, DisablesQueensideCastlingRights) {
    board_test::Harness board(board_test::fen::start);
    board.disable_castle(WHITE, A1);
    EXPECT_TRUE(board.can_castle(WHITE));
    EXPECT_TRUE(board.can_castle_kingside(WHITE));
    EXPECT_FALSE(board.can_castle_queenside(WHITE));
    board.disable_castle(BLACK, A8);
    EXPECT_TRUE(board.can_castle(BLACK));
    EXPECT_TRUE(board.can_castle_kingside(BLACK));
    EXPECT_FALSE(board.can_castle_queenside(BLACK));
}

TEST(BoardMoveApplicationTest, MakesAndUnmakesKnightMove) {
    board_test::Harness board(board_test::fen::start);
    board.make(Move(G1, F3));
    EXPECT_EQ(board.toFEN(), "rnbqkbnr/pppppppp/8/8/8/5N2/PPPPPPPP/RNBQKB1R b KQkq - 1 1");
    board.unmake();
...
TEST(BoardMoveApplicationTest, MakesAndUnmakesBishopCapture) {
    ...
    EXPECT_EQ(board.toFEN(),
              "r3k2r/p1ppqpb1/Bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPB1PPP/R3K2R b KQkq - 0 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::perft_position_2);
}
...
TEST(BoardMoveApplicationTest, RookCaptureDisablesCastlingRights) {
    ...
    EXPECT_EQ(board.toFEN(), "R3k2r/8/8/8/8/8/8/4K2R b Kk - 0 1");
    board.unmake();
...
TEST(BoardMoveApplicationTest, PawnDoublePushSetsEnPassantSquare) {
    ...
    EXPECT_EQ(board.toFEN(), board_test::fen::legal_en_passant_a3);
    board.unmake();
    EXPECT_EQ(board.toFEN(), fen);
}
...
TEST(BoardMoveApplicationTest, MakesAndUnmakesEnPassantCapture) {
    ...
    EXPECT_EQ(board.toFEN(), "4k3/8/8/8/8/p7/8/4K3 w - - 0 2");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::legal_en_passant_a3);
}
...
TEST(BoardMoveApplicationTest, MakesAndUnmakesKingsideCastle) {
    ...
    EXPECT_EQ(board.toFEN(), "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R4RK1 b kq - 1 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::perft_position_2);
}
...
TEST(BoardMoveApplicationTest, MakesAndUnmakesQueensideCastle) {
    ...
    EXPECT_EQ(board.toFEN(), "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/2KR3R b kq - 1 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::perft_position_2);
}
...
TEST(BoardMoveApplicationTest, KingMoveDisablesCastlingRights) {
    ...
    EXPECT_EQ(board.toFEN(), "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R2K3R b kq - 1 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::perft_position_2);
}
...
TEST(BoardMoveApplicationTest, KingsideRookMoveDisablesKingsideCastlingRights) {
    ...
    EXPECT_EQ(board.toFEN(), "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3KR2 b Qkq - 1 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::perft_position_2);
}
...
TEST(BoardMoveApplicationTest, MakesAndUnmakesQueenPromotion) {
    ...
    EXPECT_EQ(board.toFEN(), "Q3k3/8/8/8/8/8/8/4K3 b - - 0 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::white_pawn_on_a7);
}

TEST(BoardMoveApplicationTest, MakesAndUnmakesUnderpromotion) {
    board_test::Harness board(board_test::fen::white_pawn_on_a7);
    board.make(Move(A7, A8, MOVE_PROM, BISHOP));
    EXPECT_EQ(board.toFEN(), "B3k3/8/8/8/8/8/8/4K3 b - - 0 1");
    board.unmake();
    EXPECT_EQ(board.toFEN(), board_test::fen::white_pawn_on_a7);
}
