...
void expect_move_round_trip(std::string_view before, Move move, std::string_view after) {
    board_test::Harness board(before);
    board.make(move);
    EXPECT_EQ(board.to_fen(), after);
    board.unmake();
    EXPECT_EQ(board.to_fen(), before);
}

} // namespace

TEST(BoardMoveApplicationTest, MakesAndUnmakesKnightMove) {
    expect_move_round_trip(board_test::fen::start,
                           Move(G1, F3),
...
TEST(BoardMoveApplicationTest, RookCaptureDisablesCastlingRights) {
    ...
    expect_move_round_trip(fen, Move(A1, A8), "R3k2r/8/8/8/8/8/8/4K2R b Kk - 0 1");
}
...
TEST(BoardMoveApplicationTest, MakesAndUnmakesQueenPromotion) {
    expect_move_round_trip(board_test::fen::white_pawn_on_a7,
