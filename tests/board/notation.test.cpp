...
TEST(BoardNotationTest, FormatsDiagnosticBoard) {
    const Board board(board_test::fen::kings_only);

    EXPECT_EQ(std::format("{}", board),
              R"(   +---+---+---+---+---+---+---+---+
   |   |   |   |   | k |   |   |   | 8
   +---+---+---+---+---+---+---+---+
   |   |   |   |   |   |   |   |   | 7
...
TEST(BoardNotationTest, FormatsSanCaptures) {
...
TEST(BoardNotationTest, DisambiguatesSanByFileRankAndSquare) {
...
TEST(BoardNotationTest, IgnoresIllegalMoveWhenDisambiguatingSan) {
    expect_san("4r1k1/8/8/8/8/5N2/4N3/4K3 w - - 0 1", Move(F3, D4), "Nd4");
}

TEST(BoardNotationTest, FormatsSanPromotions) {
