...
TEST(UciParserTest, ParsesGoSupportedLimits) {
    ...
    EXPECT_EQ(command.parameters.depth, 3);
    EXPECT_EQ(command.parameters.movetime, Milliseconds::rep{20});
    EXPECT_EQ(command.parameters.nodes, NodeCount{1000});
    EXPECT_EQ(command.parameters.wtime, Milliseconds::rep{3000});
    EXPECT_EQ(command.parameters.btime, Milliseconds::rep{4000});
    EXPECT_EQ(command.parameters.winc, Milliseconds::rep{12});
    EXPECT_EQ(command.parameters.binc, Milliseconds::rep{13});
    EXPECT_EQ(command.parameters.movestogo, 5);
}
...
TEST(UciParserTest, ParsesGoNumericBoundaries) {
    ...
    EXPECT_EQ(zero.parameters.nodes, NodeCount{0});
    EXPECT_EQ(wide.parameters.nodes, wide_value);
    EXPECT_EQ(max.parameters.nodes, max_value);
    ...
    EXPECT_EQ(signed_limits.parameters.depth, std::numeric_limits<int>::max());
    EXPECT_EQ(signed_limits.parameters.movetime, std::numeric_limits<Rep>::max());
}
...
TEST(UciParserTest, RejectsInvalidGoNumericValuesWithoutConsumingLaterKeywords) {
    ...
    for (const auto value : invalid_values) {
        ...
        EXPECT_FALSE(command.parameters.nodes.has_value());
        EXPECT_EQ(command.parameters.unknown_tokens, std::vector<std::string>{std::string(value)});
    }

    const auto missing = parse_as<uci::GoCommand>("go nodes");
    EXPECT_FALSE(missing.parameters.nodes.has_value());
    EXPECT_TRUE(missing.parameters.unknown_tokens.empty());
    ...
    EXPECT_FALSE(out_of_range.parameters.depth.has_value());
    EXPECT_FALSE(out_of_range.parameters.movetime.has_value());

    const auto recovery = parse_as<uci::GoCommand>("go depth nodes 42 movetime -50");
    EXPECT_FALSE(recovery.parameters.depth.has_value());
    EXPECT_EQ(recovery.parameters.nodes, NodeCount{42});
    EXPECT_EQ(recovery.parameters.movetime, Milliseconds::rep{-50});
    EXPECT_TRUE(recovery.parameters.unknown_tokens.empty());
}
...
TEST(UciParserTest, RecordsUnsupportedGoAndTerminalSearchmoves) {
    ...
    EXPECT_TRUE(command.parameters.infinite);
    EXPECT_TRUE(command.parameters.ponder);
    EXPECT_EQ(command.parameters.mate, 4);
    ASSERT_EQ(command.parameters.unknown_tokens.size(), 1U);
    EXPECT_EQ(command.parameters.unknown_tokens[0], "unknown");
    ASSERT_TRUE(command.parameters.searchmoves.has_value());
    EXPECT_EQ(*command.parameters.searchmoves, (std::vector<std::string>{"e2e4", "d2d4"}));

    const auto absent = parse_as<uci::GoCommand>("go depth 1");
    EXPECT_FALSE(absent.parameters.searchmoves.has_value());

    const auto empty = parse_as<uci::GoCommand>("go searchmoves");
    ASSERT_TRUE(empty.parameters.searchmoves.has_value());
    EXPECT_TRUE(empty.parameters.searchmoves->empty());

    const auto terminal = parse_as<uci::GoCommand>("go searchmoves e2e4 depth 3");
    ASSERT_TRUE(terminal.parameters.searchmoves.has_value());
    EXPECT_EQ(*terminal.parameters.searchmoves, (std::vector<std::string>{"e2e4", "depth", "3"}));
    EXPECT_FALSE(terminal.parameters.depth.has_value());
}
