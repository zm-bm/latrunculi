...
TEST(Eval_isolatedPawns, StartPositionNoIsolatedPawns) {
    Board b(STARTFEN);
    EXPECT_EQ(Eval::isolatedPawns(b.pieces<PAWN>(WHITE)), 0);
    EXPECT_EQ(Eval::isolatedPawns(b.pieces<PAWN>(BLACK)), 0);
}
TEST(Eval_isolatedPawns, IsolatedPawnsOnA2AndG7) {
    Board b("rnbqkbnr/ppppp1p1/8/8/8/8/P1PPPPPP/RNBQKBNR w KQkq - 0 1");
    EXPECT_EQ(Eval::isolatedPawns(b.pieces<PAWN>(WHITE)), BB::set(A2));
    EXPECT_EQ(Eval::isolatedPawns(b.pieces<PAWN>(BLACK)), BB::set(G7));
}
...
TEST(Eval_doubledPawns, StartPositionNoDoubledPawns) {
    Board b(STARTFEN);
    EXPECT_EQ(Eval::doubledPawns<WHITE>(b.pieces<PAWN>(WHITE)), 0);
    EXPECT_EQ(Eval::doubledPawns<BLACK>(b.pieces<PAWN>(BLACK)), 0);
}
