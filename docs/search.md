...
3. For deterministic search-tree candidates, run a second fresh-process 200-position corpus pass
   and require exact candidate-internal agreement in the reproducibility fields above.
...
and OpenBench require explicit authorization in the current request or active `/goal`;
...
runner/book hashes, identical paired settings and adjudication, an enforced game cap, raw log,
PGN, pentanomial counts, and the SPRT result. Apply the same upper-bound acceptance, lower-bound
rejection, and capped-inconclusive interpretation to either backend.

Run one match at a time. Predeclare its hypothesis and enforced game budget; a larger-than-default
OpenBench budget needs explicit approval, and a missing cap blocks testing. Confirm startup once
and return control without polling. Further testing after an inconclusive cap needs separate
